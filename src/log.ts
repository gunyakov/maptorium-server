...
class Log {
  ...
  async make(type:LogType, module:LogModules,  message:string) {
    if(type != LogType.info && type != LogType.error && type != LogType.warning && type != LogType.success) {
      type = LogType.info;
    }
    ...
    if(!config.log[module]) {
      console.log(red(new Date().toLocaleString("en-GB")), red.bold(format_module), red.bold("ERROR    "), red("Cant find config for this log module."));
    }
    else {
      //If enable display such type of mesage in console
      if(config.log[module][type] == LogShow.console || config.log[module][type] == LogShow.both) {
        //Show success message
        if(type == LogType.success) {
          console.log(green(new Date().toLocaleString("en-GB")), green.bold(format_module), green.bold("SUCCESS  "), green(message));
        }
        //Show error message
        if(type == LogType.error) {
          console.log(red(new Date().toLocaleString("en-GB")), red.bold(format_module), red.bold("ERROR    "), red(message));
        }
        //Show warning message
        if(type == LogType.warning) {
          console.log(yellow(new Date().toLocaleString("en-GB")), yellow.bold(format_module), yellow.bold("WARNING  "), yellow(message));
        }
        //Show info message
        if(type == LogType.info) {
          console.log(blue(new Date().toLocaleString("en-GB")), blue.bold(format_module), blue.bold("INFO     "), blue(message));
        }
      }
      //If enable display such type of mesage in console
      if(config.log[module][type] == LogShow.message || config.log[module][type] == LogShow.both) {
