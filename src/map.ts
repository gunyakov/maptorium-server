...
//Extra imports
//------------------------------------------------------------------------------
import { Tile, MapInfo, NetworkConfig } from "./interface";
//------------------------------------------------------------------------------
//General map handler
//------------------------------------------------------------------------------
class MapHandler {
  ...
  //----------------------------------------------------------------------------
  private async _getTileFromNetwork(z:number, x:number, y:number, netConfig:NetworkConfig) {
  ...
  //----------------------------------------------------------------------------
  public async download(z:number, x:number, y:number, netConfig:NetworkConfig):Promise<[code:number, response:string, size:number]> {
    let http = await this._getTileFromNetwork(z, x, y, netConfig);
    //If empty tile
    if(http.code == 404) {
      //Save empty tile
      await TileStorage.insert(z, x, y, this.storage, Buffer.alloc(0), 0, this._mapVersion);
    }
    //If tile not empty
    if(http.code == 200) {
      //Insert tile into TileStorage
      await TileStorage.insert(z, x, y, this.storage, http.response as unknown as Buffer, http.byteLength, this._mapVersion);
    }
  ...
  //----------------------------------------------------------------------------
  public async update(z:number, x:number, y:number, netConfig:NetworkConfig):Promise<[code:number, response:string, size:number]> {
    let http = await this._getTileFromNetwork(z, x, y, netConfig);
    //If empty tile
    if(http.code == 404) {
      //Update tile in TileStorage
      await TileStorage.update(z, x, y, this.storage, Buffer.alloc(0), 0, this._mapVersion);
    }
    //If tile not empty
    if(http.code == 200) {
      //Update tile in TileStorage
      await TileStorage.update(z, x, y, this.storage, http.response as unknown as Buffer, http.byteLength, this._mapVersion);
    }
    return [http.code, http.response, http.byteLength];
  }
  /**
   ...
   */
  async checkTile(z:number, x:number, y:number, getFull:boolean):Promise<[result:boolean, tile: Tile]> {
    //Try to get tile from TileStorage
    let tile = await TileStorage.getTile(z, x, y, this.storage, getFull) as Tile;
  ...
  //----------------------------------------------------------------------------
  async rewrite(z:number, x:number, y:number, tile:Buffer):Promise<boolean> {
    let byteLength = Buffer.byteLength(tile);
    //Insert or update tile in TileStorage
