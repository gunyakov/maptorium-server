//------------------------------------------------------------------------------
//CRC32 to store tiles hash in TileStorage
//------------------------------------------------------------------------------
...
//TileStorage handler
//------------------------------------------------------------------------------
const TileStorage = require("../DB/TileStorage.js");
//------------------------------------------------------------------------------
//Logging service
//------------------------------------------------------------------------------
...
class Map {
  ...
  async getTileMain (z, x, y, url, mode, config) {
    ...
    //Chech if tile exist in TileStorage
    ...
    if(netTile && netTile != 404) {
      //If need to get tile from internet and tile missing in TileStorage
      if(downloadTile && !tile) {
        //Insert tile into TileStorage
        await TileStorage.saveTile(z, x, y, this.storage, netTile.data, netTile.data.byteLength, this._mapVersion);
      }
      ...
      if(config.updateTiles && config.updateDifferent) {
        ...
        //If tile from net hash is different tile from TileStorage hash
        if(tileHash != tile.h) {
          updateTile = true;
        }
      }
      ...
      if(updateTile) {
        if(tile) {
          ...
          await TileStorage.updateTile(z, x, y, this.storage, netTile.data, netTile.data.byteLength, this._mapVersion);
        }
        else {
          //Insert tile into TileStorage
          await TileStorage.saveTile(z, x, y, this.storage, netTile.data, netTile.data.byteLength, this._mapVersion);
      ...
      return tile;
      //If tile exist in TileStorage and need update
    }
    //--------------------------------------------------------------------------
    ...
    if(netTile == 404) {
      if(config.emptyTiles) {
        if(tile) {
          //Insert or update tile in TileStorage
          await TileStorage.updateTile(z, x, y, this.storage, "", 0, this._mapVersion);
        }
        else {
          //Insert tile into TileStorage
          await TileStorage.saveTile(z, x, y, this.storage, "", 0, this._mapVersion);
        }
      }
      //Show error message
    ...
    //If tile in TileStorage and we reach this point, just return tile from TileStorage
    if(tile) return tile;
    //Show error message
  ...
  async checkTile(z, x, y, getFull) {
    ...
    let tile = await TileStorage.getTile(z, x, y, this.storage, getFull);
    //If tile is present in TileStorage
    if(tile) {
      //Set that tile was take from TileStorage
      tile.method = "db";
      //Return tile
      return tile;
    }
  ...
  async saveTile(z, x, y, tile) {
    //Insert or update tile in TileStorage
    return await TileStorage.updateTile(z, x, y, this.storage, tile.data, tile.byteLength, 0);
  }
}
