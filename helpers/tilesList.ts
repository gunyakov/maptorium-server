...
//----------------------------------------------------------------------------
export default async function tileList(
  bbox: Array<number>,
...
export async function tilesListByPOI(
  poiID: number,
  zoom: number,
  tileSize: number,
): Promise<Array<TileInfo>> {
  Log.success(
    LogModules.worker,
    `Start calculation tiles list for Polygon ${poiID} and Zoom ${zoom}`,
  );
  let arrJobTilesList: Array<TileInfo> = [];
  ...
  Log.success(
    LogModules.worker,
    `Calculation of tiles list is finished. Total ${arrJobTilesList.length} tiles.`,
  );
  return arrJobTilesList;
}

export async function tileListByPolygon(
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    let point = EPSG3857.latLngToPoint(
  ...
  if (bbox[0] == bbox[2] || bbox[1] == bbox[3]) {
    Log.warning(
      LogModules.worker,
      "Abort tiles calculation. Required Zoom is same as selected zoom.",
    );
    return arrJobTilesList;
  }
