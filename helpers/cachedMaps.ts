...
export async function getCachedMap(mapID:string, zoom:number, arrTiles:Array<TileInfo>) {
    ...
    if(mapHandler) {
        ...
        for(let i = 0; i < arrTiles.length; i++) {
            ...
            if(makeTileChecking) {
                let [tileInDB, tileInfo] = await mapHandler.checkTile(arrTiles[i].z as number, arrTiles[i].x, arrTiles[i].y, false);
        ...
        if(makeTileChecking) {
            ...
            Log.success(LogModules.main, `Finished checking tiles in DB for cached map. Time spend ${time}.`);
            time = Date.now();
...
export async function getCachedMapByPOI(poiID:number, zoom: number, map:string) {
    let tileList = await tilesListByPOI(poiID, zoom);
    if(tileList.length > 0) {
        getCachedMap(map, zoom, tileList);
    }
}

export async function getCachedMapByBBOX(bbox:Array<number>, zoom: number, map:string) {
    ...
    points.push({lat: bbox[1], lng: bbox[0]});
    let tileList = await tileListByPolygon(points, zoom);
    if(tileList.length > 0) {
        getCachedMap(map, zoom, tileList);
    }
}