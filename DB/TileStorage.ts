...
class TileStorage {
  ...
  //----------------------------------------------------------------------------
  async getDB(z:number, x:number, y:number, storage:string, createDB:boolean = false) {
  ...
  //Get tile from DB
  //----------------------------------------------------------------------------
  async getTile(z:number, x:number, y:number, storage:string, getFull:boolean = false ) {
  ...
  //Save tile in DB
  //----------------------------------------------------------------------------
  async insert(z:number, x:number, y:number, storage:string, blob:Buffer, size:number, mapVersion = 0) {
     ...
     if(config.db.ReadOnly === false) {
      ...
      let results = await DB.run(dbName, "INSERT_TILE", [x, y, mapVersion, "", size, Math.abs(bstr(blob.toString('utf8'))), timeStamp, blob]);
  ...
  //Update tile in DB
  //----------------------------------------------------------------------------
  async update(z:number, x:number, y:number, storage:string, blob:Buffer, size:number, mapVersion:number = 0):Promise<boolean> {
    //Get folder + file name of DB
    let dbName = await this.getDBName(z, x, y, storage);
    //Open DB
    ...
    //Update tile in DB
    let results = await DB.run(dbName, "UPDATE_TILE", [mapVersion, size, Math.abs(bstr(blob.toString('utf8'))), timeStamp, blob, x, y]) as boolean;
    //Если запрос вернул результат
    if(results) {
      //Make log
