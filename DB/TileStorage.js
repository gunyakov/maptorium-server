...
class TileStorage {
  ...
  async getTile(z, x, y, storage, getFull) {
    ...
    //If request return undefined state
      ...
      //Make log
      Log.error("TSTOR", "request problem in " + dbName);
  ...
  async updateTile(z, x, y, storage, blob, size, mapVersion = 0) {
    ...
    if(tile !== false) {
      ...
      if(results) {
        //Make log
        Log.success("TSTOR", "UPDATE -> " + dbName);
        //Return
        return true;
      }
    }
...
module.exports = new TileStorage();
